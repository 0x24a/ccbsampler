...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        if fixed_length:
            ...
            x = F.interpolate(
                x, size=(2 * h + 1, 2 * w + 1), mode="bilinear", align_corners=True
            )
            x = x[:, :, :-1, :-1]
...
class ASPPModule(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Sequential(
            Mean(dim=-2, keepdims=True), Conv2DBNActiv(nin, nout, 1, 1, 0, activ=activ)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.cat(
            (feat1, self.conv2(x), self.conv3(x), self.conv4(x), self.conv5(x)), dim=1
        )
